...
- **功能**：插件启用时调用，完成所有初始化。
- **实现逻辑**：
  1. 保存单例 `instance = this`。
  2. 依次创建 `ConfigRegistry`、`MessageManager`、`VaultManager`。
  3. 预加载 `config` 和 `lang` 配置（通过 `configRegistry.get(...)` 触发文件创建和加载）。
  4. 调用 `setupCommands()` 注册命令。
  5. 注册事件监听器 `Untitled1EventListener`。
  6. 发送启用消息 `plugin_on_enabled` 到控制台。
  7. 根据 `vaultManager.isEnabled()` 输出经济挂钩状态日志。
- **注意**：
  - 管理器创建顺序：`ConfigRegistry` 必须先于 `MessageManager`，因为 `MessageManager` 构造时需要从 registry 获取 `lang` 配置。
  - 预加载 `config` 和 `lang` 确保后续使用时文件已存在。
  - 事件监听器注册必须在 `setupCommands()` 之后或之前均可，无依赖。

#### `onDisable()`

- **功能**：插件卸载时调用，清理资源。
- **实现逻辑**：
  1. 若 `messageManager` 非空，发送禁用消息 `plugin_on_disabled` 到控制台。
  2. 若 `configRegistry` 非空，调用 `saveAll()` 保存所有已加载的配置。
- **注意**：保存配置确保服主修改的配置不会丢失。

#### `setupCommands()`

- **功能**：初始化 ACF 命令管理器并注册命令类。
- **实现逻辑**：
  1. `commandManager = new PaperCommandManager(this)`。
  2. `commandManager.enableUnstableAPI("help")` 启用帮助功能（必须）。
  3. 加载 ACF 语言文件：设置默认语言为 `Locale.ENGLISH`，从 `lang_en.yml` 加载本地化键值对，开启 `usePerIssuerLocale(true)`（根据玩家客户端语言自动切换）。
  4. `commandManager.registerCommand(new Untitled1Command(this))` 注册命令类。
- **注意**：
  - ACF 语言文件加载可能失败（文件不存在或格式错误），此时仅输出警告，不影响命令注册。
  - `enableUnstableAPI("help")` 即使名称带 "unstable"，实际上是稳定功能，必须调用才能使用 `@HelpCommand`。

#### `reload()`

- **功能**：重载所有配置和语言文件，由 `/untitled1 reload` 命令调用。
- **实现逻辑**：
  1. `configRegistry.reloadAll()` 重载所有已加载的配置文件（包括 `config` 和 `lang`）。
  2. `messageManager.reload()` 重新读取 `lang.yml` 并更新前缀。
  3. 发送 `reload_success` 消息到控制台。
- **注意**：`messageManager.reload()` 会重新从磁盘加载 `lang.yml`，而 `configRegistry.reloadAll()` 也会重载 `lang`，但两者不冲突（`MessageManager` 持有的是同一个 `ConfigManager` 实例，第二次重载是重复操作）。实际可优化为只调用 `messageManager.reload()` 即可。
...
- **功能**：初始化字段。
- **实现**：`this.plugin = plugin; this.msg = plugin.getMessageManager();`
- **注意**：依赖注入，便于单元测试。
...
- **参数**：`JavaPlugin plugin, String fileName`（不含 `.yml` 后缀）
- **实现**：
  1. `this.plugin = plugin; this.fileName = fileName + ".yml";`
  2. 调用 `reload()` 立即加载。
...
- **功能**：从磁盘重新加载配置。
- **实现**：
  1. 若 `configFile` 为 `null`，创建 `File(plugin.getDataFolder(), fileName)`。
  2. 若文件不存在，调用 `plugin.saveResource(fileName, false)` 复制默认文件。
  3. `config = YamlConfiguration.loadConfiguration(configFile)`。
- **注意**：`saveResource` 的第二个参数 `false` 表示如果文件已存在则覆盖，但此方法仅在文件不存在时调用，所以安全。

#### `save()`

- **功能**：将当前配置保存到磁盘。
- **实现**：若 `config` 和 `configFile` 均非空，尝试 `config.save(configFile)`，捕获 `IOException` 并记录严重日志。
...
- **实现**：
  ...
  2. `this.langConfig = plugin.getConfigRegistry().get("lang");`
  3. 调用 `reload()` 初始化前缀。
